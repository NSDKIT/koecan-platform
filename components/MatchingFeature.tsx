...
export function MatchingFeature() {
  ...
  return (
    <div>
      ...
      {isLoading && (
        <div className="processing-container text-center p-8">
      ...
      {!isLoading && results.length > 0 && (
         <div className="space-y-6">
            {results.map((rec, index) => (
                rec && (
                <article key={index} className="company-card border border-gray-200 rounded-xl p-6">
                    <header className="mb-4">
                    <h3 className="text-xl font-bold text-gray-800 mb-3">{rec.company.company_name}</h3>
                    <div className="score-display space-y-2">
                        <div className="flex items-center">
                          <span className="text-sm text-gray-700 w-24">成長可能性:</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-2 mr-2">
                            <div className="bg-orange-500 h-2 rounded-full" style={{width:`${rec.scores.growth*10}%`}}></div>
                          </div>
                          <span className="text-sm font-semibold">{rec.scores.growth}/10</span>
                        </div>
                        <div className="flex items-center">
                          <span className="text-sm text-gray-700 w-24">文化フィット:</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-2 mr-2">
                            <div className="bg-orange-500 h-2 rounded-full" style={{width:`${rec.scores.culture*10}%`}}></div>
                          </div>
                          <span className="text-sm font-semibold">{rec.scores.culture}/10</span>
                        </div>
                        <div className="flex items-center">
                          <span className="text-sm text-gray-700 w-24">働きやすさ:</span>
                          <div className="flex-1 bg-gray-200 rounded-full h-2 mr-2">
                            <div className="bg-orange-500 h-2 rounded-full" style={{width:`${rec.scores.wlb*10}%`}}></div>
                          </div>
                          <span className="text-sm font-semibold">{rec.scores.wlb}/10</span>
                        </div>
                    </div>
                    </header>
                    <section className="match-points mb-4">
                    <h4 className="font-semibold text-gray-800 mb-2">あなたへの推薦理由</h4>
                    <p className="text-gray-700" dangerouslySetInnerHTML={{ __html: rec.personalizedReason }} />
                    </section>
                    <div className="action-links">
                        <a href="#" className="primary-action text-orange-600 hover:text-orange-700 font-semibold">企業の詳細を見る</a>
                    </div>
                </article>
                )
            ))}
...
function simulateClaudeScoring(geminiOutput: any, db: Company[]) {
    const scoredCompanies = db.map(company => {
        ...
        if (!locationMatch) scores.culture -= 5;
        if (!industryMatch) scores.growth -= 5;
