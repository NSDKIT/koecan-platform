...
import { X, Gift, Send, Loader2, Mail, MessageSquare } from 'lucide-react';
...
export function PointExchangeModal({ currentPoints, onClose, onExchangeSuccess }: PointExchangeModalProps) {
  ...
  const handleExchange = async () => {
    ...
    if (pointsAmount < selectedOption.minPoints) {
    ...
    try {
        ...
        .insert([
          {
            ...
            amount: -pointsAmount,
            reason: 'exchange',
            description: `ポイント交換リクエスト: ${exchangeType} ${pointsAmount}pt`
          },
        ] as any);
      ...
      const { error: requestError } = await supabase
        ...
        .insert([
          {
            user_id: user.id,
            reward_name: availableExchangeOptions.find(opt => opt.type === exchangeType)?.name || '',
            points_used: pointsAmount,
            provider: exchangeType,
            status: 'processing',
            contact_info: contactType === 'email' ? contactInfo : 'LINE連携済み',
